...
public class Application extends Controller {
	...
	public static String poi(String poiId) throws IllegalArgumentException, IllegalAccessException, ClassNotFoundException, NoSuchMethodException, SecurityException, InvocationTargetException,
	...
	public static String powerTagDetails(String powerTag, String poiId) throws IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException, ClassNotFoundException,
			NoSuchMethodException, InvocationTargetException {
		Poi poi = Poi.findById(Long.valueOf(poiId));
		Object poiPowerTag = Poi.class.getField("powerTag").get(poi);

		if (poiPowerTag.getClass() != Class.forName("models.powerTags." + powerTag)) {
			poiPowerTag = null;
		}

		Field[] fields = Class.forName("models.powerTags." + powerTag).getFields();
		return getHtmlCode(fields, poiPowerTag, "", "");
	}

	public static String getMethodHtmlCode(String source, String poiId) throws IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException, ClassNotFoundException,
			NoSuchMethodException, InvocationTargetException {
		Poi poi = Poi.findById(Long.valueOf(poiId));
		Object poiPowerTag = Poi.class.getField("powerTag").get(poi);
	...
	public static String getTypeHtmlCode(String source, String method, String poiId) throws IllegalArgumentException, IllegalAccessException, NoSuchFieldException, SecurityException,
	...
	private static String getHtmlCode(Field[] fields, Object objectWithValues, String source, String method) throws IllegalArgumentException, IllegalAccessException, ClassNotFoundException,
		...
		for (Field field : fields) {
			...
			if (!type.equals("interface java.util.List") && !name.equals("id") && !name.equals("willBeSaved") && !name.contains("powerTag")) {

				String select = "<label for='" + name + "'>" + name.replace("_", " ").replace("T", " t") + "</label>";
				...
				if (type.contains("Generator$MethodEnum") && fields.length == 1) {
					...
					if (methods.length > 1) {
						for (MethodEnum methodEnum : methods) {
							output += "<option value='" + methodEnum.name() + "'>" + methodEnum.name + "</option>";
						}
						output += "</select>";
					...
					if (types.length > 1) {
						for (TypeEnum typeEnum : types) {
							output += "<option value='" + typeEnum.name() + "'>" + typeEnum.name + "</option>";
						}
						output += "</select>";
					...
					for (Object enumConstant : enumConstants) {
						...
						output += "<option value='" + enumType + "'>" + enumName + "</option>";
					}
					output += "</select>";
				...
				} else if (!type.contains("Generator$MethodEnum") && !type.contains("Generator$TypeEnum") && !type.contains("class models.Poi")) {
					String value = "";
					...
					if (type.equals("class models.types.Operator")) {
						...
						output += getHtmlCode(Operator.class.getFields(), operator, source, method);
						output += "<p>Operator END</p>";
					} else {
						String lowerCaseType = type.toLowerCase();
						String parsleyValidator = "";
						if (lowerCaseType.equals("byte") || lowerCaseType.equals("double") || lowerCaseType.equals("class java.lang.float") || lowerCaseType.equals("float")
								|| lowerCaseType.equals("int") || lowerCaseType.equals("long")) {
							parsleyValidator = "data-parsley-type='number'";
						}
						output += "<p>";
						output += "<label for='" + name + "'>" + name.replace("_", " ").replace("T", " t") + "</label>";
						if (name.toLowerCase().contains("time") || name.toLowerCase().contains("date")) {
							...
							output += "<input type='text' " + parsleyValidator + " id='" + name + "' name='" + name + "' value='" + value + "' class='submitPoiDataField'/>";
						}
						output += "</p>";
					}
				}
			} else if (field.toString().equals("public java.util.List models.Poi.photos")) {
				ArrayList list = new ArrayList((List) field.get(objectWithValues));

				for (int i = 0; i < list.size(); i++) {
					output += "<img width='200' src='/application/getpicture?position=" + i + "&id=" + ((Poi) objectWithValues).id.toString() + "' alt='image of poi with id "
							+ ((Poi) objectWithValues).id.toString() + " on position " + i + "'/>";
				}
			}
		}
	...
	public static void updatePoi(String accuracy, String altitude, String bearing, String id, String latitude, String longitude, String provider, String time) {
		Poi poi = Poi.findById(Long.valueOf(id));
